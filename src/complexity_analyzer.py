...
class ComplexityAnalyzer:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.complexity_indicators = self.config.get("complexity", {}).get("indicators", {})
    ...
    def analyze_complexity(self, goal: str, context_files: List[str] = None) -> ComplexityLevel:
        """
        ...
        final_complexity = self._combine_complexity_scores(goal_complexity, context_complexity)
    ...
    def _analyze_goal_complexity(self, goal: str) -> ComplexityLevel:
        ...
        high_matches = sum(1 for indicator in high_indicators if indicator in goal_lower)
        medium_matches = sum(1 for indicator in medium_indicators if indicator in goal_lower)
    ...
    def _combine_complexity_scores(self, goal_complexity: ComplexityLevel, 
                                 context_complexity: ComplexityLevel) -> ComplexityLevel:
    ...
    def get_model_recommendation(self, complexity: ComplexityLevel) -> str:
        ...
        model_mapping = {
            ComplexityLevel.LOW: self.config.get("models", {}).get("efficient", "claude-3-5-haiku-latest"),
            ComplexityLevel.MEDIUM: self.config.get("models", {}).get("advanced", "claude-3-5-sonnet-latest"),
            ComplexityLevel.HIGH: self.config.get("models", {}).get("premium", "gpt-4o-2024-08-06"),
            ComplexityLevel.ENTERPRISE: self.config.get("models", {}).get("premium", "gpt-4o-2024-08-06")
        }
        
        return model_mapping[complexity]
    
    def get_token_limit(self, complexity: ComplexityLevel, role: str = "default") -> int:
        """Get token limit based on complexity and role."""
        base_limits = {
            ComplexityLevel.LOW: self.config.get("guards", {}).get("efficient_max_output_tokens", 1200),
            ComplexityLevel.MEDIUM: self.config.get("guards", {}).get("advanced_max_output_tokens", 3200),
            ComplexityLevel.HIGH: self.config.get("guards", {}).get("premium_max_output_tokens", 4000),
