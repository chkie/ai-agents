...
class BudgetMonitor:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.task_warning_threshold = self.budget_config.get(
            "task_budget_warning_eur", 2.0
        )
    ...
    def get_monthly_spend(self, date: Optional[datetime] = None) -> float:
    ...
    def show_budget_warning_if_needed(self) -> bool:
        ...
        if status["daily_utilization"] >= 80:
            ...
            self.console.print(
                f"[yellow]Daily budget: {status['daily_spend']:.3f}€ / {status['daily_budget']}€ ({status['daily_utilization']:.1f}%)[/yellow]"
            )
            self.console.print(
                f"[yellow]Recommendation: Consider using --force-model claude-3-5-haiku-latest for cost efficiency[/yellow]\n"
            )
            return True
        return False
    ...
    def get_cost_analysis(self, days: int = 7) -> Dict[str, Any]:
        ...
        recent_costs = [entry for entry in self.costs if entry.timestamp > cutoff_str]
        ...
        complexity_costs = {}
        for entry in recent_costs:
            complexity_costs[entry.complexity] = (
                complexity_costs.get(entry.complexity, 0) + entry.cost_eur
            )
    ...
    def generate_budget_report(self) -> str:
        ...
        daily_status = (
            "🔴 EXCEEDED"
            if status["daily_utilization"] >= 100
            else "🟡 WARNING" if status["daily_warning"] else "🟢 OK"
        )
        monthly_status = (
            "🔴 EXCEEDED"
            if status["monthly_utilization"] >= 100
            else "🟡 WARNING" if status["monthly_warning"] else "🟢 OK"
        )
