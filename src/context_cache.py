...
class ContextCache:
    ...
    def _cleanup_old_sessions(self):
        ...
        if len(self.sessions) > self.max_sessions:
            sorted_sessions = sorted(
                self.sessions.items(), key=lambda x: x[1].last_used, reverse=True
            )
    ...
    def _has_file_changed(self, file_path: Path, cached_entry: FileEntry) -> bool:
        ...
        return (
            mtime != cached_entry.mtime
            or size != cached_entry.size
            or content_hash != cached_entry.content_hash
        )
    ...
    def get_cached_context(
        self, repo_path: str, file_specs: List[str], goal: str
    ) -> Optional[str]:
        ...
        actual_files = expand_context_specs(
            repo_path, file_specs, limit=self.max_files_per_session
        )
        ...
        if cache_valid and session.files:
            ...
            console.print(
                f"[green]📦 Cache hit: {len(session.files)} files loaded from cache[/green]"
            )
    ...
    def cache_context(
        self, repo_path: str, file_specs: List[str], goal: str, context_content: str
    ):
        ...
        if len(actual_files) > self.max_files_per_session:
            console.print(
                f"[yellow]⚠️ Warning: {len(actual_files)} files exceed cache limit of {self.max_files_per_session}[/yellow]"
            )
            console.print(
                f"[yellow]Consider using more specific file patterns to reduce token usage[/yellow]"
            )
                ...
                try:
                    ...
                    if file_marker in context_content:
                        ...
                        if end_idx == -1:
                            ...
                            file_content = context_content[
                                start_idx + len(file_marker) : end_idx
                            ].strip()
        ...
        console.print(
            f"[blue]💾 Cached {len(cached_files)} files for future use[/blue]"
        )
    ...
    def reset_cache(self, session_id: Optional[str] = None):
        ...
        if session_id:
            if session_id in self.sessions:
                ...
                console.print(
                    f"[yellow]🗑️ Reset cache for session {session_id}[/yellow]"
                )
            else:
    ...
    def list_sessions(self):
        ...
        for sid, session in self.sessions.items():
            ...
            console.print(
                f"  {status} [bold]{sid}[/bold] ({session.file_count} files, {age_hours:.1f}h old)"
            )
